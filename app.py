...
def api_stats():
    ...
    try:
        print("📊 Computing photo library statistics (metadata only)...")
        
        # Get basic photo count WITHOUT expensive analysis (just metadata)
        all_photos, excluded_count = scanner.get_unprocessed_photos(include_videos=False)
        total_photos = len(all_photos)
        print(f"📊 Found {total_photos} photos ({excluded_count} excluded) - no expensive analysis performed")
        ...
        # Return basic stats WITHOUT doing expensive analysis
        # No grouping or analysis until user clicks "Analyze My Photos"
        
        return jsonify({
            'success': True,
            'total_photos': total_photos,
            'potential_groups': "Click 'Analyze My Photos' to find duplicates",
            'estimated_savings': "Analysis not run yet",
            'timestamp': datetime.now().isoformat()
        })
...
def api_analyze_cluster(cluster_id):
    ...
    try:
        ...
        final_groups = scanner.filter_groups_by_visual_similarity(enhanced_groups, similarity_threshold=50.0)
        ...
        if priority and hasattr(scanner, 'cached_clusters') and scanner.cached_clusters:
            ...
            if (cached_groups is not None and cached_timestamp is not None and 
                ...
                if force_real or from_filters:
                    ...
                    groups = scanner.filter_groups_by_visual_similarity(groups, similarity_threshold=50.0)
...
def api_analyze_filtered():
    """DISABLED: Use filter → legacy workflow instead of this direct endpoint.
    ...
    """
    print("⚠️ DEPRECATED ENDPOINT: /api/analyze-filtered should not be used")
    print("👆 Use the proper filter → legacy workflow instead")
    
    return jsonify({
        'success': False,
        'error': 'This endpoint is deprecated. Please use the filter → legacy workflow instead.',
        'redirect': '/filters',
        'message': 'Navigate to /filters, apply your criteria, then go to /legacy and click "Analyze My Photos"'
...
def api_complete_workflow():
    ...
    try:
        ...
        # Get photo details for export - ONLY for the specific photos being deleted
        db = scanner.get_photosdb()
        photo_lookup = {}
        for uuid in photo_uuids:
            try:
                ...
                if photo:
                    photo_lookup[uuid] = photo
            except Exception as e:
                print(f"⚠️ Could not get photo {uuid} for export: {e}")
        print(f"📤 Export lookup ready: {len(photo_lookup)} photos indexed for deletion (fast lookup)")
